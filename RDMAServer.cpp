...
class RDMAException : public runtime_error {
  public:
...
RDMAServer::RDMAServer(const uint16_t port) : RDMAServer(std::string(), port) {}

RDMAServer::RDMAServer(const std::string& host, const uint16_t port) : RDMAServer() {
  ...
  assert(port_assigned == port); //TODO: throw?
